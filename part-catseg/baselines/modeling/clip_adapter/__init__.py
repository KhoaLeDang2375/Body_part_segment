...
from .adapter import ClipAdapter, MaskFormerClipAdapter, PerPixelClipAdapter, \
    MaskFormerObjPartClipAdapter, MaskFormerObjPartClipAdapterWithObjectGuidance
...
def build_prompt_learner(cfg):
    if cfg.PROMPT_LEARNER == "predefined":
        ...
        if cfg.PROMPT_CHECKPOINT != "":
            checkpoint = torch.load(cfg.PROMPT_CHECKPOINT, map_location="cpu", weights_only=False)["model"]
